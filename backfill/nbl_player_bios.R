...
# zz <- cbind(nbl_players) |> data.frame()
# yy <- zz |> unnest_wider(nbl_players)
# 
# bb <- rbind(nbl_players) |> data.frame()
...
save_to_rel(df=test_df, file_name = "nbl_player_bios", ext = "csv", release_tag = "csvs")
save_to_rel(df=test_df, file_name = "nbl_player_bios", ext = "rds", release_tag = "csvs")
